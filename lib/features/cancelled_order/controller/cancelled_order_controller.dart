...
class CancelledOrderController extends GetxController {
  ...
  Future<void> fetchCancelledOrders({bool isRefresh = false}) async {
    try {
      ...
      logger.i('Fetching cancelled orders page ${currentPage.value}');

      final response = await orderFeedService.fetchCancelledOrders(
        page: currentPage.value,
        limit: 20,
      );

      if (isFirstPage) {
        cancelledOrders.assignAll(response.data);
      } else {
        cancelledOrders.addAll(response.data);
      }
      ...
      logger.e('Error fetching cancelled orders', error: e, stackTrace: stack);
      if (cancelledOrders.isEmpty) {
        errorMessage.value = 'Failed to load cancelled orders';
      } else {
        EasyLoading.showError('Failed to load more cancelled orders');
      }
    } finally {
  ...
  Future<void> refreshOrders() async {
    await fetchCancelledOrders(isRefresh: true);
  }
