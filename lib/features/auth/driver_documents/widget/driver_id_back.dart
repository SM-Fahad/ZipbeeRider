...
class IdentityBackScreen extends StatelessWidget {
  const IdentityBackScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("NRIC (Back)")),
        ...
        child: Column(
          children: [
            ...
            Obx(() {
              ...
              if (controller.idBack.value != null) {
                image = Image.file(controller.idBack.value!, fit: BoxFit.cover);
              } else if (controller.idBackUrl.isNotEmpty) {
                image = Image.network(
                  controller.idBackUrl.value.startsWith('http')
                      ? controller.idBackUrl.value
                      : '${ApiEndPoint.baseUrl}/${controller.idBackUrl.value}',
                  fit: BoxFit.cover,
            ...
            const Text("Make sure the ID card back side is visible"),
            ...
            _button(
              Icons.image,
              "Upload photo",
              () => controller.pickImage(controller.idBack),
            ),
          ],
        ),
      ),
